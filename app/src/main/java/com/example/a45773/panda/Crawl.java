package com.example.a45773.panda;
...
//					e.printStackTrace();
	 ...
	 */
	public void MessageDecode(String messageStr) throws JSONException {
		...
		if (type.equals(Danmu.Code)) {//普通消息
			Danmu danmu=(Danmu) messageReceied;
			msgbean.setName(danmu.mNickName);
			msgbean.setType("chatmsg");
			msgbean.setText(danmu.mContent);
			/*msgbean.setLv(lv);
			msgbean.setRg(rg);*/
		} else if (type.equals(Gift.Code)) {//普通礼物
			Gift gift=(Gift) messageReceied;

			msgbean.setGfid(gift.mContentName);
			msgbean.setGfcnt(Integer.parseInt(gift.mContentCount));
			msgbean.setName(gift.mNickName);
			msgbean.setType("dgb");
			/*msgbean.setLv(lv);
			msgbean.setRg(rg);*/
		} else if (type.equals(Bamboo.Code)) {//竹子
			Bamboo gift=(Bamboo) messageReceied;

			msgbean.setGfid("竹子");
			msgbean.setGfcnt(Integer.parseInt(gift.mContent));
			msgbean.setName(gift.mNickName);
			msgbean.setType("dgb");
			/*msgbean.setLv(lv);
			msgbean.setRg(rg);*/
    ...
    @Override
    public void run() {
		if (!mIsHeartBeatThreadStop)
			return;
    	ConnectToDanMuServer(MainActivity.id+"");
    }
}
